...
class TabbedCalc01Model {
    ...
    func performOperation() -> String {
        ...
        // adding the current calculation into an array that holds the history of the calculations
        let currCalculation = [decimalToBinary(decimalNum: self.firstOperand), self.operation, decimalToBinary(decimalNum: self.secondOperand), decimalToBinary(decimalNum: computation)]
        calculationList.append(currCalculation)
