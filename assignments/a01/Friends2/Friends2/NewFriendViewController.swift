    ...
    @IBAction func addFriend(_ sender: Any) {
        if let currAppDelegate = UIApplication.shared.delegate as? AppDelegate {
            ...
            if (self.firstName != nil && self.lastName != nil && self.email != nil) {
                ...
                if let lAddFriendNagivationViewController = self.navigationController {
                    
                    if let lSplitViewControllers = lAddFriendNagivationViewController.splitViewController {
                        
                        if lSplitViewControllers.viewControllers.count >= 2 {
                            if let lFriendNavigationController = lSplitViewControllers.viewControllers[1] as? UINavigationController {
                                
                                if let lFriendTableViewController = lFriendNavigationController.viewControllers[0] as? UITableViewController {

                                    if let lFriendTableView = lFriendTableViewController.view as? UITableView {
                                        lFriendTableView.reloadData()
                                    }
                                }
                            }
                        }
